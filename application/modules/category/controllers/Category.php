...
class Category extends CI_Controller {
	...
	public function index()
    {
		...
		$data['data']=$this->model->ambildata($this->db->dbprefix('category'));
		$data['content'] = 'category/list';
		$data['css'] = 'category/css';
		$data['javascript'] = 'category/js';
	...
	public function edit($idcategory)
    {
		...
		$data['datauser']=$this->model->ambildataById($this->db->dbprefix('users'),'iduser',$data['iduser']);
		$data['data']=$this->model->ambildataById($this->db->dbprefix('category'),'idcategory',$data['idcategory']);
		$data['datalanguage']=$this->model->ambildata($this->db->dbprefix('language'));
	...
	public function saves()
    {
		...
		$config['upload_path'] = './assets/files/category/';
		$config['allowed_types'] = 'png|jpg';
		$config['max_size'] = '20480';
		$image = $_FILES['image']['name'];
		$path="./assets/files/category/";
		...
		{	
			...
			$config['source_image'] = './assets/files/category/'.$image;
		...
		$this->model->simpandata($this->db->dbprefix('category'),$data);
	...
	public function updates()
    {
		...
		if(!empty($_FILES['image']['name']))
		{
			...
			{	
				$queryimg = $this->db->query("SELECT image FROM `".$this->db->dbprefix('category')."` WHERE idcategory='".$idcategory."'");
				$row = $queryimg->row();
				$picturepath="./assets/files/category/".$row->image;	
				unlink($picturepath);
				...
				$thumb_file_path="./assets/files/category/".$thumb_file;
		...
		$clause=array('idcategory'=>$idcategory);
		$this->model->update($this->db->dbprefix('category'),$data,$clause);
